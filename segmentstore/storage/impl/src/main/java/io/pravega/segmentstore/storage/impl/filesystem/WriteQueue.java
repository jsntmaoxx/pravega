...
/**
 * A specialized queue for FileSystem writes. Provides methods for adding new items, determining the next items to execute,
...
public class WriteQueue {
    ...
    WriteQueue(Supplier<Long> timeSupplier) {
        ...
        this.stats = new AtomicReference<>(new QueueStats(0, 0, FileSystemConfig.MAX_APPEND_LENGTH, 0));
    }
    ...
    private void updateStats(long lastDurationMillis) {
        ...
        this.stats.set(new QueueStats(this.writes.size(), this.totalLength, FileSystemConfig.MAX_APPEND_LENGTH, (int) lastDurationMillis));
    }
     ...
     * * The writes to return have the same File Path assigned as the first write in the queue.
     *
    ...
    synchronized List<Write> getWritesToExecute(long maximumAccumulatedSize) {
        ...
        // Collect all remaining writes, as long as they are not currently in-progress and have the same file path
        // as the first item in the file.
        Path firstFile = this.writes.peekFirst().getWriteFile().metadata.getFile();
        boolean canSkip = true;

        List<Write> result = new ArrayList<>();
        for (Write write : this.writes) {
            if (accumulatedSize >= maximumAccumulatedSize) {
                // Reached the throttling limit.
                break;
            }
            ...
            if (write.isInProgress()) {
                if (!canSkip) {
                    ...
                    // This is likely due to a bunch of writes failing, but we overlapped
                    // with updating their status. Try again next time (when that write completes).
                    return Collections.emptyList();
                }
            } else if (write.getWriteFile().metadata.getFile().compareTo(firstFile) != 0) {
                // We cannot initiate writes in a new file until all writes in the previous file completed.
                break;
